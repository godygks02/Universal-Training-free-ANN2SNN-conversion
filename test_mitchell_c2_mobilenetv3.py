...
def download_imagenet_samples():
    """Downloads 5 sample test images from public URLs."""
    samples = [
        {
    ...
    loaded_samples = []
    print("\nDownloading 5 sample validation images...")
    for s in samples:
        try:
    ...
    if len(loaded_samples) < 5:
        print("  Warning: Download failed. Generating random synthetic images for missing samples.")
        while len(loaded_samples) < 5:
            idx_missing = len(loaded_samples)
            s = samples[idx_missing]
...
def main():
    ...
    for idx, sample in enumerate(samples):
        ...
        # Calibration & Shape Extraction
        calibration_ranges, layer_shapes = calibrate_and_collect_shapes(ann_model, img_tensor, device)
        ...
        snn_model = replace_mobilenet_modules_with_approx(
            snn_model, calibration_ranges, timesteps=16, prefix_k=3, device=device
        )
        ...
        print(f"\nSample {idx+1}: {target_name}")
        print(f"  - ANN Top-1: {class_labels[ann_top1_idx]} ({ann_top5_vals[0, 0].item()*100:.2f}%)")
        print(f"  - SNN Top-1: {class_labels[snn_top1_idx]} ({snn_top5_vals[0, 0].item()*100:.2f}%)")
        print(f"  - SNN Energy Savings: {(1.0 - energy['snn_total'] / energy['ann_total']) * 100:.1f}%")
    ...
    # 6. Plot 1: Classification Probability Comparison (5 Rows x 2 Columns)
    fig, axes = plt.subplots(5, 2, figsize=(13, 18))
    
    for r, data in enumerate(plot_data):
        # Column 0: Image
