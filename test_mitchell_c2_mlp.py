...
# ─────────────────────────────────────────────────────────────────────────────
# 3. IEEE 754 Exponent-Guided S-PLA LayerNorm
# ─────────────────────────────────────────────────────────────────────────────
...
class SPLALayerNorm(nn.Module):
    ...
    and final scaling utilizing Exponent-Guided Bit-Slice spiking interaction.
    """
    def __init__(self, normalized_shape, eps=1e-5, s_val=3.0, timesteps=16):
        ...
        # Exponent guided bit slice encoder
    ...
    def forward(self, x):
        ...
        # (Replacing the expensive T x T dynamic spike interaction)
        ...
        # Apply standard LayerNorm weight and bias
        return out * self.weight + self.bias


# ─────────────────────────────────────────────────────────────────────────────
# 4. Spike-Driven S-PLA Activation Wrapper
# ─────────────────────────────────────────────────────────────────────────────
...
class SPLAActivationWrapper(nn.Module):
    ...
    def forward(self, x):
        # S-PLA forward: returns (out, spikes, seg_idx, x_rec_prefix) when return_details=True
        out_step, spikes, _, _ = self.spla(x, return_details=True)
        
        with torch.no_grad():
            # Use absolute sum since spikes are ternary {-1, 0, 1}
